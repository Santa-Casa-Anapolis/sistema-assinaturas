...
const dbPath = path.join(__dirname, 'temp-database.sqlite');
const db = new sqlite3.Database(dbPath);

// Simular pool para compatibilidade
const pool = {
  query: (sql, params = []) => {
...
async function initDatabase() {
  try {
    console.log('✅ Inicializando banco SQLite temporário...');
    ...
    for (const user of users) {
      try {
        await pool.query(`
          INSERT OR IGNORE INTO users (name, email, username, role, password, sector, is_admin)
          VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    for (const group of defaultGroups) {
      try {
        await pool.query(`
          INSERT OR IGNORE INTO access_groups (name, description, permissions)
          VALUES (?, ?, ?)
    ...
    for (const sector of defaultSectors) {
      try {
        await pool.query(`
          INSERT OR IGNORE INTO sectors (name, folder_path)
          VALUES (?, ?)
